...
- Метакей-фильтрация (`_bookmarks`, `_techOverride`, `_hidden`, `name`, `author`, `uuid`,
  `description`) выносится в общий помощник и применяется до передачи строк в игру.
...
1. На загрузке читает все таблицы переводов из известной папки (напр.
   `Mods/Config/UltimaLoc/*.json`) и их манифесты (имя целевой сборки).
...
- Требуется Harmony, доступный в среде MSCLoader (HarmonyX поставляется с MSCLoader).
...
- Реальные конвенции путей MSCLoader для размещения мода и конфигов (структура §4 —
  предварительная).
- Доступность/версия HarmonyX в целевой среде MSCLoader.
