...
- [ ] 1. Общий помощник фильтрации мета-ключей перевода
...
- [ ] 2. Обобщить обработчик `MOD_REPACK` в игро-независимый слой
  ...
  - Хендлер принимает `gameId`, резолвит `games.getGameModule(gameId)`, применяет
    `stripMetaKeys` к `updatedData` и вызывает `game.pack(input, ctx)`.
...
- [ ] 3. Реализовать BG3 `pack()` поверх существующего `saveAndRepack` (без регрессий)
...
- [ ] 4. Расширить `assemblyInfo.js`: чтение имени целевой сборки
...
- [ ] 5. Построитель таблицы переводов MSC (чистая функция)
...
- [ ] 5.1 Property-тесты построителя таблицы (Property 2, Property 3)
...
- [ ] 6. MSC pack-менеджер: каркас и переразрешение исходника
...
- [ ] 7. MSC режим `replace` (fallback через `inject`)
...
- [ ] 7.1 Интеграционный тест round-trip замены (Property 4, Property 6)
...
- [ ] 8. Конфиг и дистрибуция патчера `UltimaLocPatcher`
...
- [ ] 9. Включить патчер в систему зависимостей (статус-виджет + блокировка)
...
- [ ] 10. MSC режим `patch`: формирование артефакта
...
- [ ] 11. Frontend: проброс параметров упаковки
...
- [ ] 12. Frontend: выбор режима для MSC (patch/replace)
...
- [ ] 13. Frontend: прогресс и итог упаковки MSC
...
- [ ] 14. [ULTIMA_TOOLS] C#-патчер `UltimaLocPatcher` (MSCLoader-мод)
...
- [ ] 14.1 [ULTIMA_TOOLS] Юнит-тесты патчера (Property 5)
...
- [ ] 14.2 [ULTIMA_TOOLS] Сборка и релиз патчера
...
- [ ] 15. Тесты устойчивости к обновлению оригинала (Property 7)
...
- [ ] 16. Регрессионная проверка BG3 и финальная сборка
