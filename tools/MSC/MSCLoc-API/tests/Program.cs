...
Console.WriteLine("\nLocStore format templates - reverse-match a formatted runtime string");

Test("reverse-matches a single-placeholder template and substitutes the capture", () =>
{
    LocStore.Reset();
    LocStore.AddTemplate("{0} achievements remaining.", "{0} достижений осталось.");
    Assert(LocStore.TryTranslateBlock("77 achievements remaining.", out var tr), "should match the template");
    Assert(tr == "77 достижений осталось.", "wrong substitution: " + tr);
    // A different value flows through the same template.
    Assert(LocStore.TryTranslateBlock("1 achievements remaining.", out var tr2) && tr2 == "1 достижений осталось.", "second value wrong: " + tr2);
});

Test("template does not over-match unrelated text", () =>
{
    LocStore.Reset();
    LocStore.AddTemplate("{0} achievements remaining.", "{0} достижений осталось.");
    Assert(!LocStore.TryTranslateBlock("Start a new game", out _), "unrelated text must not match");
});

Test("reorders placeholders by index when the translation moves them", () =>
{
    LocStore.Reset();
    LocStore.AddTemplate("{0} of {1} done", "готово {1}: {0}");
    Assert(LocStore.TryTranslateBlock("3 of 10 done", out var tr), "should match");
    Assert(tr == "готово 10: 3", "placeholder order not respected: " + tr);
});

Test("a near-empty template (just a placeholder) is rejected to avoid matching everything", () =>
{
    LocStore.Reset();
    LocStore.AddTemplate("{0}", "ПЕРЕВОД {0}");
    Assert(!LocStore.TryTranslateBlock("anything at all", out _), "bare-placeholder template must be ignored");
});

Test("a direct (whole-string) match still wins over template matching", () =>
{
    LocStore.Reset();
    LocStore.Map[LocId.Make("Loaded.")] = "Загружено.";
    LocStore.AddTemplate("{0} Loaded.", "{0} Загружено.");
    Assert(LocStore.TryTranslateBlock("Loaded.", out var tr) && tr == "Загружено.", "direct match should win: " + tr);
});

Test("Hello banner: nested-tag template captures the (Steam) name and keeps it", () =>
{
    LocStore.Reset();
    LocStore.AddTemplate(
        "<b><color=orange>Hello <color=lime>{0}</color>!</color></b>",
        "<b><color=orange>Привет, <color=lime>{0}</color>!</color></b>");

    // Name present (the real GetPersonaName() case).
    Assert(LocStore.TryTranslateBlock("<b><color=orange>Hello <color=lime>Nikon</color>!</color></b>", out var tr),
        "should match the Hello template");
    Assert(tr == "<b><color=orange>Привет, <color=lime>Nikon</color>!</color></b>",
        "name not substituted; got: " + tr);

    // Empty name (Steam offline) still translates, just with an empty capture.
    Assert(LocStore.TryTranslateBlock("<b><color=orange>Hello <color=lime></color>!</color></b>", out var tr2),
        "empty-name variant should still match");
    Assert(tr2 == "<b><color=orange>Привет, <color=lime></color>!</color></b>", "empty-name got: " + tr2);
});

Console.WriteLine("\nLocStore.LoadFromJson - source-keyed 'strings' table (MSCLoader RU authoring)");

Test("'strings' pairs translate by content, with case variants and templates derived", () =>
{
    LocStore.Reset();
    string json = "{\"targetAssembly\":\"MSCLoader\",\"strings\":{\"Mods\":\"Моды\",\"{0} mods loaded\":\"загружено модов: {0}\"}}";
    Assert(LocStore.LoadFromJson(json), "should parse the table");
    Assert(LocStore.Targets.Contains("MSCLoader"), "targetAssembly must register");
    Assert(LocStore.TryTranslateBlock("Mods", out var a) && a == "Моды", "direct source translate: " + a);
    Assert(LocStore.TryTranslateBlock("MODS", out var b) && b == "Моды", "UPPER variant must resolve: " + b);
    Assert(LocStore.TryTranslateBlock("3 mods loaded", out var c) && c == "загружено модов: 3", "template reverse-match: " + c);
});
